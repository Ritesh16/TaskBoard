...
import type { TaskDetailFormData } from "../schemas/taskDetailSchema";
...
export const useTasks = (taskId?: number) => {
    ...
    const saveTaskDetails = useMutation({
        mutationFn: async (taskDetail: TaskDetailFormData) => {
            const response = await agent.post('/tasks/SaveTaskDetails', taskDetail);
            return response.data;
        },
        onSuccess: async () => {
            await queryClient.invalidateQueries({
